...
sigma.mf <- read_csv('outputs/sims1_sigma_MF.csv')%>%
  arrange(sub_id)
...
sigma.mb <- read_csv('outputs/sims1_sigma_MB.csv')%>%
  arrange(sub_id)
...
sigma.mfmb <- read_csv('outputs/sims1_sigma_MFMB.csv')%>%
  arrange(sub_id)
...
no_sigma.MFMB <- read_csv('outputs/sims1__no_sigma_MFMB.csv')%>%
  arrange(sub_id)
...
rand_sigma.MFMB <- read_csv('outputs/sims1_random_sigma_MFMB.csv')%>%
  arrange(sub_id)
...
sjPlot::tab_model(model.sigmaMF, model.sigmaMB, model.no_sigmaMFMB, model.sigmaMFMB, model.rand_sigmaMFMB, file = 'outputs/simulations_logreg.doc')
